...
import { setRequiresPasswordReset } from "../../features/auth/authSlice";
...
const VerifyOtp = () => {
  ...
  const isReady = !!recipient && !!window.confirmationResult;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (dispatchResult.meta.requestStatus === "fulfilled") {
        ...
        if (!verifiedUser) {
          const errorMessage =
            typeof dispatchResult.payload === "string"
              ? dispatchResult.payload
              : dispatchResult.payload?.error ||
                "OTP verification failed. Try again.";

          setInputError(errorMessage);
          toast.error(errorMessage);
          return; // 🔴 Don't continue if user is missing
        }

        setSuccess(true);
        toast.success("OTP Verified Successfully!");

        // ✅ Clear confirmationResult + recaptcha
        ...
        setTimeout(() => {
          const verifiedUser = dispatchResult.payload.user;
          if (!verifiedUser) return navigate("/");
          const {
            role,
            onboardingComplete,
          ...
          console.log(
            "[OTP Verified] requiresPasswordReset:",
            verifiedUser?.requiresPasswordReset
          );

          if (role === "provider") {
            if (!onboardingComplete) return navigate("/onboarding");
            if (hasMechanicProfile) return navigate("/select-role");
            return navigate("/provider/dashboard");
          }

          if (role === "mechanic") {
            if (user?.mechanic?.isSelfRegistered) {
              return navigate("/mechanic/dashboard"); // self-registered skips password reset
            }
            if (requiresPasswordReset) return navigate("/set-password");

...
export default function MechanicFormDialog({
  ...
  useEffect(() => {
    ...
    if (open && mechanicId && mechanicId !== "self") {
      dispatch(fetchMechanicById(mechanicId));
    }

    if (open && mechanicId === "self" && authUser) {
  ...
  useEffect(() => {
    ...
    } else if (mechanicId !== "self" && selected?.photo) {
      setPhotoPreview(getPhotoUrl(selected.photo));
    } else {
      setPhotoPreview(null);
    }
  }, [formik.values.photo, selected, mechanicId]);

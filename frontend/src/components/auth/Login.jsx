...
export default function Login({ onSwitch }) {
  ...
  const { loading, token, user, hydrated } = useSelector((state) => state.auth);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (next === "set-password" && user?.requiresPasswordReset) {
        toast.info("Please set your new password.");
        navigate("/set-password");
        return;
      }

      // ✅ Dual-role: show role selector
      if (next === "select-role") {
        toast.success("Login successful. Please choose your role.");
      ...
      switch (user.role) {
        case "admin":
          ...
          break;
        case "mechanic":
          toast.success(`Welcome ${user.name || "Mechanic"}`);
          navigate(getMechanicRedirect(user));
          break;
        case "provider":
          toast.success(`Welcome ${user.name || "Provider"}`);
